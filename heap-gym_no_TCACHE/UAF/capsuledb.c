...
int del_index;
int view_index;
int total_capacity = sizeof(storage_pointers)/sizeof(storage_pointers[0]); // Double free fix
void* freed_list[100]; // Double Free fix
int nofree = 0; // Double Free fix
int freecounter; // Double Free Fix
...
if (choice == 1){
  ...
  for (int i=0; i <= total_capacity; i++){
    if (storage_pointers[del_index] == freed_list[i]){
      printf(" [x] Double-Free Attempt: Freed Already! Can't free already free chunk! \n\n");
      fflush(stdout);
      nofree = 1;
    }
  }
